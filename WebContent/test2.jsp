...
<!-- ---Hien thi so sach--- -->
		...
		if(session.getAttribute("gh")!=null){
			...
			session.setAttribute("soSach", (int)gh00.demSach());
		}
		int soSach1;
		...
		%>	
  <h2>Số sách trong giỏ: <%=soSach1%></h2>
<!-- ---Hien thi so sach--- -->
  <h2>Nhà sách</h2>
...
<table >
	<tr>
				...
				<%for(loaibean loai: lbo.getloai()) {%>
                    <tr>
                      <td class="btn btn-info">
                        <a style="color: white" href="HTSach.jsp?ml=<%=loai.getMaloai() %>"> <%//link :ml được request ở dòng 69 sau đó chạy ở dòng 72 %>
		...
		<tr>
			...
			<td align="center">
				<form method="post" >
						<input class="btn btn-primary" type="submit" name="selectall" value="Chọn tất cả"  />
				</form>
			</td>
		</tr>
			...
			for(GioHangbean g1: gh1.ds){%>
				<tr>
					<td width="50px" height="70px">
						<img src=image_sach/<%=sbo.TimAnh(ds1, g1.getMasach())%> width="100%" height="100%" >
					</td>
					
					<td valign="bottom">
						
					<form method="post" action="EditSach.jsp">
					<%out.print(g1.getMasach() +": "+g1.getTensach());%>
						<br>							
						<%out.print("Giá: "+g1.getGia() +"đ x ");%> 
						<input name="slmua" value="<%=g1.getSlmua() %>" type="number" style="width:50px"> 
						<input name="MaSach" value="<%=g1.getMasach()%>" type="hidden"> 
						<input class="btn btn-success" name="sub0" value="Cập nhật" type="submit"> 
						<input class="btn btn-info" name="sub1" value="Xoá" type="submit">	
					</form>
					</td>
					<td align="center">
						
					<form method="post" >
						<input name="<%=g1.getMasach()%>" type="checkbox" <%if(request.getParameter("selectall")!=null){out.print("checked");} %> />
					</form>
					</td>					
		...
		<td valign="top" width="200"> 	
               <p>Tìm kiếm</p>
				...
				<form >
				    ...
				    <input class="btn btn-danger" name="butt2" value="Đồng ý" type="submit">				 	
				</form>	
		</td>
	</tr>
</table>
